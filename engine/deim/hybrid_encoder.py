...
class ConvFFN(nn.Module):
    """
    创新模块: 卷积前馈网络
    相比纯 Linear，增加了 3x3 DWConv 来增强对小目标边缘特征的提取。
    """
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, dropout=0.):
        ...
        # 创新点: Depthwise Convolution 用于提取局部特征
        # groups=hidden_features 保证了参数量极小
    ...
    def forward(self, x, H, W):
        ...
        # Reshape 为 2D 图像进行卷积处理
        x_img = x.transpose(1, 2).view(B, -1, H, W)
        x_img = self.dwconv(x_img)
        x = x_img.flatten(2).transpose(1, 2)  # 还原为 [B, N, C]
...
class PositionalEncodingGenerator(nn.Module):
    """
    创新模块: CPE (Conditional Positional Encoding)
    替代静态 SinCos，动态生成位置信息，对小目标定位更精准。
    """

    def __init__(self, in_channels):
        super(PositionalEncodingGenerator, self).__init__()
        # 使用 3x3 卷积隐式编码位置
        self.proj = nn.Conv2d(in_channels, in_channels, 3, 1, 1, groups=in_channels)

    def forward(self, x):
        return x + self.proj(x)

# 改进
class SmallObjectEncoderBlock(nn.Module):
    def __init__(self,
                 d_model,
                 nhead,
                 dim_feedforward=2048,
                 ...
                 normalize_before=True):  # 推荐默认为 True，训练更稳定
        super().__init__()

        # 1. 动态位置编码 (替代 SinCos)
        self.pos_generator = PositionalEncodingGenerator(d_model)
        ...
        # 3. 创新 ConvFFN
        self.norm2 = nn.LayerNorm(d_model)
        self.conv_ffn = ConvFFN(d_model, dim_feedforward, dropout=dropout, act_layer=nn.GELU)
    ...
    def forward(self, src):
        ...
        # --- Step 1: 注入动态位置信息 (无需 mask, 无需 grid) ---
        src = self.pos_generator(src)

        # 展平 [B, C, H, W] -> [B, HW, C]
        x = src.flatten(2).transpose(1, 2)

        # --- Step 2: Self-Attention (Pre-Norm 结构) ---
        # 小目标技巧: Pre-Norm 在深层网络中对梯度传播更友好
        residual = x
        if self.normalize_before:
            x = self.norm1(x)

        # 注意: CPE 已经将位置信息融入 x，所以 q=k=v=x
        x, _ = self.self_attn(x, x, x)
        x = residual + self.dropout(x)
        if not self.normalize_before:
            x = self.norm1(x)

        # --- Step 3: ConvFFN (局部特征增强) ---
        residual = x
        if self.normalize_before:
            x = self.norm2(x)

        # 传入 H, W 以便内部还原图像结构做卷积
        x = self.conv_ffn(x, h, w)
